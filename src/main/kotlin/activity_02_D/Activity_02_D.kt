...
fun main() {
    ...
    var money = IntArray(5)
    var total = 0
    for (i in money.indices) {
        print("Input money no. ${i + 1}: ")
        var inputMoney = readln()?.toInt()
        money.set(i, inputMoney!!)
        total += money[i]
    }
    print("Divide $total by?: ")
    var quotient = readln().toInt()
    if (quotient != 0) {
        var divided = total / quotient
        println("$total divided by $quotient = $divided")
    } else {
